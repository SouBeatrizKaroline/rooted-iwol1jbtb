...
export function InteractiveMap({ selectedRouteIndex = 0, onSelectRoute }: InteractiveMapProps) {
  ...
  if (loading)
    return (
      <div className="w-full h-[380px] lg:h-[480px] bg-secondary border border-border rounded-xl flex items-center justify-center">
  ...
  return (
    <div className="relative w-full h-[380px] lg:h-[480px] bg-secondary border border-border rounded-xl overflow-hidden flex flex-col">
      <svg
      ...
      >
        <defs>
          <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
            <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#DCE3DC" stroke-width="1" />
          </pattern>
        </defs>
        ...
        {layers.recommended && (
          <path
            ...
            fill="none"
            stroke={selectedRouteIndex === 0 ? '#2F6B45' : '#DCE3DC'}
        ...
        <path
          ...
          fill="none"
          stroke={selectedRouteIndex === 1 ? '#D6A84F' : '#DCE3DC'}
        ...
        <path
          ...
          fill="none"
          stroke={selectedRouteIndex === 2 ? '#3B82F6' : '#DCE3DC'}
      ...
      <div className="relative z-10 flex-1">
        {filtered.map((loc) => (
          ...
          >
            <span
              className="block w-3 h-3 rounded-full border-2 border-card shadow-lg transition-transform group-hover:scale-125"
      ...
      <div className="relative z-10 p-2 bg-card/90 border-t border-border flex items-center gap-3 text-xs">
        <button
          onClick={() => onSelectRoute?.(0)}
          className="flex items-center gap-1.5 hover:opacity-80"
        >
          <span className="w-3 h-1 bg-primary rounded-full" />
          <span className="text-[11px] text-foreground">Route B</span>
        </button>
        <button
        ...
        >
          ...
          <span className="text-[11px] text-muted-foreground">Route A</span>
        </button>
      </div>

      {selected && (
        <div className="absolute bottom-12 left-3 right-3 md:right-auto md:w-80 bg-card border border-border rounded-xl p-3 shadow-elevation z-20 animate-fade-in-up">
          ...
          <Button size="sm" variant="outline" className="w-full mt-2 h-7 text-[11px]">
            View Details
          </Button>
        </div>
      )}
